...
import 'package:cash_flow/features/transactions/ui/widget/expense_radar_chart.dart';
...
class _CashFlowSummaryCardState extends State<CashFlowSummaryCard>
  ...
  void _editSalary(BuildContext context) {
    ...
    TransactionModel? currentMonthSalaryTransaction;
    try {
      ...
      builder: (context) => _EditSalaryDialog(
        ...
        onSalaryUpdated: () {
          context.read<TransactionBloc>().add(
                TransactionEvent.loadTransactions(userId: widget.userId),
              );
          final now = DateTime.now();
          final startDate = DateFormatter.getStartOfMonth(now);
          final endDate = DateFormatter.getEndOfMonth(now);
  ...
  void _saveSalary() {
    if (_formKey.currentState!.validate()) {
      ...
      if (widget.existingTransactionId != null) {
        context.read<TransactionBloc>().add(
              TransactionEvent.deleteTransaction(transactionId: widget.existingTransactionId!),
            );
      }
      ...
      context.read<TransactionBloc>().add(
            TransactionEvent.addTransaction(transaction: salaryTransaction),
          );
